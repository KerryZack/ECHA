...
def vllm_batch_inference_fs(args, jsonl_filename):
    """
    ...
    # 读取jsonl文件
    print(f"[FS推理] 读取jsonl文件: {jsonl_filename}")
    jsonl_data = []
    ...
    print(f"[FS推理] 共 {len(jsonl_data)} 条数据需要推理")
    ...
    # 批量推理
    print(f"[FS推理] 开始批量推理（批次大小: {args.batch_size}）...")
    all_responses = []
    ...
    if use_message_format:
        print(f"[FS推理] 检测到Qwen-VL模型，将使用message协议格式")
    ...
    print(f"\n[FS推理] 输入文件目录: {parent_dir_name}")
    print(f"[FS推理] 输出目录: {output_dir}")
    print(f"[FS推理] 保存推理结果: {inference_result_filename}")
    ...
    print(f"[FS推理] ✓ 完成！推理了 {len(all_responses)} 条数据")
    print(f"[FS推理] 推理结果文件: {inference_result_filename}")
    print(f"{'='*60}\n")
...
def main():
    parser = argparse.ArgumentParser(description="FS方法推理脚本")
    
    # 输入文件参数
    parser.add_argument("--jsonl-file", type=str, 
                        default='/path/to/workspace/models/FigStep/safebench_400.jsonl',
                       help="输入的jsonl文件路径")
    ...
    # 执行批量推理
    vllm_batch_inference_fs(args, args.jsonl_file)
